var Controller = new function () {
    ...
    function getModel() {
        var model = {
            ...
            ReportId:callarOptions.Report.Id,
            ParentId:callarOptions.model.Id
        };
        console.log([callarOptions,callarOptions.model]);
        return model;
    };
    function save() {
        if (formModel.IsValid) {
            var model = getModel();
            windowModel.Wait();
            Global.CallServer('/AccountantArea/AccountReportingItem/Create', function (response) {
                windowModel.Free();
                if (!response.IsError) {
                    cancel();
                    callarOptions.OnSuccess();
                } else {
                    Global.Error.Show(response, '/AccountantArea/AccountReportingItem/Create');
                }
            }, function (response) {
                windowModel.Free();
                response.Id = -8;
    ...
    function getDropdown() {
        ...
        var group=getAccountDpr('GroupId', '/AccountantArea/AccountGroup/AutoComplete', gropFlt, function (data) {
            if (data && data.Id) {
                acntFlt.value = data.Id;
            } else {
                acntFlt.value = '00000000-0000-0000-0000-000000000000';
            }
            acntDpr.Reload();
        });
        acntDpr = getAccountDpr('AccountGroupId', function () {
            return '/AccountantArea/AppAccount/AutoCompleteForReport?reportId=' + callarOptions.Report.Id
        }, acntFlt, function (data) {
    ...
    this.Show = function (opts) {
        ...
        if (windowModel) {
            ...
            Global.LoadTemplate('/Content/IqraPharmacy/AccountantArea/Templates/AccountReportingItem/Add.html', function (response) {
