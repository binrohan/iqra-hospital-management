...
var Controller = new function (none) {
    ...
    function save() {
        ...
        if (model.IsValid) {
            ...
            Global.Uploader.upload({
                data: model,
                url: '/NonePharmacy/NonePharmacyItemReceive/AddNew',
    ...
    (function (nope) {
        ...
        function summaryDiscountChanged() {
            ...
            formModel.Discount = (100 * tDiscount / tTPrice).toFloat(4);
            formModel.MarginDiscount = (100 * (tSPrice - tdPrice) / tdPrice).toFloat(4);
        };
        ...
        function rowBound(elm) {
            ...
            //$(td[1]).html(getInput('BatchName', 'BatchName', noop));
            selfService.Unit.Set($(td[1]), this);
            selfService.UnitQuentity.Set($(td[2]), this);
            selfService.UnitSalePrice.Set($(td[3]), this);
            selfService.UnitTradePrice.Set($(td[4]), this);
            selfService.TotalPrice.Set($(td[5]), this);
            selfService.VAT.Set($(td[6]), this);
            selfService.Discount.Set($(td[7]), this);
            //selfService.Date.Set($(td[8]), this);
            $(td[8]).html(Global.Click($('<button class="btn btn_cancel btn-white btn-default btn-round" type="button"><span class="glyphicon glyphicon-remove"></span> </button>'), onRemove, this));
        ...
        this.Bind = function () {
            Global.Grid.Bind({
                ...
                columns: [{ field: 'Name', title: 'Trade Name' },
                    //{ field: 'BatchName', autobind: false },
                    { field: 'SalesUnitType', title: 'Unit(S/P)' },
                    ...
                    { field: 'UnitQuentity', title: 'Unit Qnt', className: 'unit_quentity', autobind: false },
        ...
        this.GetData = function () {
            ...
            gridModel.dataSource.each(function (position) {
                ...
                list.push({
                    ...
                    Type: 0,//this type is for Purchase, Return, cancel and 0 for Purchase
                    MarginDiscount: 100 * (this.SUnitSalePrice * this.SaleUnitQuentity - totalTradePrice) / totalTradePrice
                });
        ...
        (function () {
            function onchange(model) {
                summaryDiscountChanged();
                model.FormModel.PayablePrice = model.PayablePrice = (model.TotalTradePrice + model.VatTotal - model.DiscountTotal).toFloat(5);
                console.log(model);
            }
            function onDiscountChanged(model) {
                model.Discount = parseFloat(model.FormModel.Discount).toFloat(5)||0;
                model.FormModel.DiscountTotal = model.DiscountTotal = Math.round(model.TotalTradePrice * model.Discount).div(100).toFloat(5);
                onchange(model);
            }
            function onDiscountTotalChanged(model) {
                model.DiscountTotal = parseFloat(model.FormModel.DiscountTotal).toFloat(5)||0;
                if (model.TotalTradePrice)
                    model.FormModel.Discount = model.Discount = (100 * model.DiscountTotal / model.TotalTradePrice).toFloat(5);
                else
        ...
        (function () {
            ...
            function onVatChanged(model) {
                model.Vat = parseFloat(model.FormModel.Vat || '0').toFloat(5)||0;
                model.FormModel.VatTotal = model.VatTotal = model.TotalTradePrice.mlt(model.Vat).div(100).toFloat(5);
                onchange(model);
            }
            function onVatTotalChanged(model) {
                model.VatTotal = parseFloat(model.FormModel.VatTotal).toFloat(5)||0;
                if (model.TotalTradePrice)
                    model.FormModel.Vat = model.Vat = model.VatTotal.mlt(100).div(model.TotalTradePrice).toFloat(5);
                else
