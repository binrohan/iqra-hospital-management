var Controller=AddEmployeeLeave = new function () {
    ...
    function save() {
        if (formModel.IsValid) {
            windowModel.Wait();
            var model = getModel();
            Global.CallServer('/HRIS/Leave/AddNewByAdmin', function (response) {
                if (!response.IsError) {
                    windowModel.Free();
    ...
    this.Show = function (opts) {
        ...
        if (windowModel) {
            ...
            Global.LoadTemplate('/Content/IqraPharmacy/HRIS/Templates/Leave/AddEmployeeLeaveByAdmin.html', function (response) {
    ...
    this.Events = new function () {
        ...
        function bindDropDown() {
            drp = { elm: $(formInputs['CategoryId']), Id: 'CategoryId', url: '/HRIS/LeaveCategory/AutoComplete', change: onDropDownChange };
            Global.AutoComplete.Bind(drp);
            Global.AutoComplete.Bind({ elm: $(formInputs['EmployeeId']), url: '/EmployeeArea/Employee/AutoComplete' });
        }
