var Controller = new function () {
    ...
    this.Show = function (opts) {
        ...
        if (windowModel) {
            ...
            Global.LoadTemplate('/Areas/SalaryArea/Templates/Salary/GenerateSalarySheet.html', function (response) {
    ...
    (function () {
        ...
        function setData(models, from, to) {
            var list = setDates(models, from, to);
            setWeekend(list);
    ...
    (function () {
        ...
        function onChange(formModel,model) {
            setAbsent(model);
        ...
        function getMaxHours(minutes, model) {
            return getFloat(model.WorkingDay.Minutes) - ((getFloat(model.Attendance.Minutes) +
                getFloat(model.LeaveItem.Minutes) +
                getFloat(model.LateEntry.Minutes)) - minutes);
        };
        function onTotalChange(formModel, handler, model) {
            formModel.TotalValue = (getFloat(formModel.Total || '0') || 0).toFloat();
            //formModel.Total = formModel.TotalValue.toMoney();
            onChange(formModel, model);
        };
        function onDayChange(formModel, handler, model,changeModel) {
            console.log([formModel, handler, model, changeModel]);
            changeModel = changeModel || {};
            var rate = changeModel.HourlyRate ||getFloat(formModel.HourlyRate || '0') || 0,
                day = getFloat(formModel.Day || '0') || 0,
                minutes = parseInt(day.mlt(model.WorkingDay.DailyMinutes)),
                    prevHour = formModel.Hour;
            formModel.Hour = minutes.mlt(60);
            if (!isValid(model)) {
                minutes = getMaxHours(minutes, model);
                formModel.Hour = minutes.mlt(60);
                formModel.Day = parseInt(hour.div(model.WorkingDay.DailyHours), 10);
            }
            formModel.TotalValue = (rate * minutes/60).toFloat();
            formModel.Total = formModel.TotalValue.toMoney();
            onChange(formModel, model);
        };
        function onHourlyRateChange(formModel, handler, model) {
        ...
        function onHourChange(formModel, handler, model) {
            var rate = getFloat(formModel.HourlyRate || '0') || 0,
                hour = getFloat(formModel.Hour || '0') || 0;
        ...
        function setFunc(key, inputs, formModel, handler, model, changeModel) {
            handler = handler || {};
            ...
            $(inputs[key]).keydown(function () {
                ...
                if (formModel[key] != handler.PrevModel[key]) {
                    (handler['on' + key + 'Change'] || changer['on' + key + 'Change']).call(this, formModel, handler, model, changeModel);
                }
            });
        };
        ...
        function getAllowance(container, model, handler, baseModel) {
            ...
            for (var key in inputs) {
                //(key === 'Day' || key === 'Hour') &&
                setFunc(key, inputs, model.FormModel, handler, baseModel);
            }
            container.empty().append(elm);
        };
        (function () {
            this.Create = function (model, container) {
                ...
                var inputs = getRow(container, dataModel, this, { Total: true }, model);
                ...
                if (model.WorkingDay.Minutes > 0) {
                    dataModel.MinuteRate = _formModel.MinuteRate = model.EmployeeDetails.Salary.div(dataModel.Minutes);
                    dataModel.HourlyRate = dataModel.MinuteRate.mlt(60)
                    _formModel.HourlyRate = dataModel.HourlyRate.toMoney();
                }
                else {
                    dataModel.MinuteRate = _formModel.MinuteRate = 0;
                    _formModel.HourlyRate = dataModel.HourlyRate = 0;
                }
            };
        ...
        (function () {
            this.Create = function (model, container) {
                ...
                dataModel.MinuteRate = model.WorkingDay.MinuteRate
                dataModel.HourlyRate = model.WorkingDay.HourlyRate;
                _formModel.HourlyRate = dataModel.HourlyRate.toMoney();
                if (model.WorkingDay.Minutes > 0) {
                    _formModel.TotalValue = dataModel.MinuteRate.mlt(dataModel.Minutes).toFloat();
        ...
        (function () {
            this.Create = function (model, container) {
                ...
                if (dataModel.Minutes > 0) {
                    dataModel.MinuteRate = dataModel.Total.div(dataModel.Minutes);
                    dataModel.HourlyRate = dataModel.MinuteRate.mlt(60);
                    _formModel.HourlyRate = dataModel.HourlyRate.toMoney();
                } else {
                    dataModel.MinuteRate = model.WorkingDay.MinuteRate;
