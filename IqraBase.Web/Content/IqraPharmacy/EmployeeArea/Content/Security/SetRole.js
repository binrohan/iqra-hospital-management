var Controller = new function () {
    ...
    function save() {
        if (formModel.IsValid) {
            ...
            Global.CallServer(saveUrl, function (response) {
                ...
                response.Id = -8;
                error.Save(response, saveUrl);
            }, { roleId: formModel.RoleId, UserId: callerOptions.model.Id }, 'POST');
        }
        return false;
    };
    function cancel() {
    ...
    function populate(model) {
        model = model || {};
        formModel.ActionMethode = model.FirstName +' '+ model.LastName;
    };
    var dropDownList = new function () {
        ...
        function set() {
            Global.CallServer('/Roles/DropDown', function (response) {
                if (!response.IsError) {
    ...
    function show(model, template) {
        ...
        document.title = formModel.Title = 'Set Role';
        populate(model);
        dropDownList.Bind();
    }
    this.Show = function (options) {
        ...
        if (windowModel) {
            ...
            Global.LoadTemplate('/Areas/Account/Templates/AssignRole.html', function (response) {
                Global.Free();
