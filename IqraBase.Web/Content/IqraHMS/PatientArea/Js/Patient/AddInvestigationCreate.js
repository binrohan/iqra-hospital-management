...
var Controller = new function () {
    ...
    function getModel() {
        ...
        model.DiscountType = formModel.DiscountType;
    ...
    function save() {
        if (formModel.IsValid) {
            ...
            Global.CallServer('/InvestigationArea/PatientInvestigation/OnInvestigationCreate', function (response) {
                windowModel.Free();
                if (!response.IsError) {
    ...
    function ageSection(value,title) {
        if (value == 0) {
            return ' ';
        } else if (value == 1) {
            return '1 ' + title+' ';
    ...
    function calculateAge(date) {
        var currentDate = new Date(),age='',day='1 Days', month = '',year='';
        var value = currentDate.getDate() - date.getDate(), flg = 0;
        if (value < 0) {
            day = (value + maxDays[date.getMonth()]);
            flg = -1;
        } else {
            day = value;
        }
        value = currentDate.getMonth() - date.getMonth() + flg;
        flg = 0;
        if (value < 0) {
            month = (value + 12);
            flg = -1;
        ...
        year = currentDate.getFullYear() - date.getFullYear() + flg;
        return ageSection(day, 'day') + ageSection(month, 'month') + ageSection(year, 'year');
    };
    function populate(isFirstTime) {
        ...
        //console.log(['calculateAge', callerOptions.model.DateOfBirth, callerOptions.model.DateOfBirth.getDate()]);
        formModel.Age = calculateAge(callerOptions.model.DateOfBirth.getDate());
        //printInvoice('ae1043a3-48ef-4fb3-9ae3-5c8d09a38015');
        };
    this.Show = function (model) {
        ...
        if (windowModel) {
            ...
            Global.LoadTemplate('/Content/IqraHMS/PatientArea/Templates/AddInvestigationCreate.html', function (response) {
